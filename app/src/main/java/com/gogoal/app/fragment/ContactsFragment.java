...
import com.gogoal.app.ui.view.IndexBar;
...
public class ContactsFragment extends BaseFragment {
    ...
    public void doBusiness(Context mContext) {
      setFragmentTitle("联系人");
        ...
        contactBeanList =getConstactsData();

        Collections.shuffle(contactBeanList);

        DividerItemDecoration decoration=new DividerItemDecoration(getContext(),LinearLayoutManager.VERTICAL);
        rvContacts.addItemDecoration(decoration);

        final int[] avatars={R.mipmap.ic_launcher,
                R.mipmap.cache_img_contacts_0,
                R.mipmap.cache_img_contacts_1,
                R.mipmap.cache_img_contacts_2,
                R.mipmap.cache_img_contacts_3};

        rvContacts.setAdapter(new CommonAdapter<ContactBean>(getContext(),R.layout.item_contacts,contactBeanList) {
            @Override
            protected void convert(ViewHolder holder, ContactBean contactBean, int position) {
                ...
                holder.setImageResource(R.id.item_contacts_iv_icon,avatars[(int) (Math.random()*5)]);
            }
        });

        indexBar.setOnIndexChangeListener(new IndexBar.OnIndexChangeListener() {
            @Override
            public void onIndexChange(int state, String letter) {
                tvConstactsFlag.setVisibility(state==0? View.VISIBLE:View.GONE);
                tvConstactsFlag.setText(letter);
            }
        });
    ...
    private List<ContactBean> getConstactsData() {
        ...
        for (int i=0;i<20;i++){
            ...
            contactBean.setmAccount("");
            contactBean.setmAccountId(String.valueOf(i));
            contactBean.setmAlias("");
            contactBean.setmAvatar("");
