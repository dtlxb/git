...
/**
 * @author wangjd on 2016/12/5 0005.
 * @Staff_id 1375
 * @phone 18930640263
 */
...
public class ExpandableLayout extends LinearLayout implements View.OnClickListener {
    ...
    public void setOrientation(int orientation) {
        if (LinearLayout.HORIZONTAL == orientation) {
            throw new IllegalArgumentException("ExpandableLayout的Orientation只能是垂直方向");
        }
    ...
    private void init(AttributeSet attrs) {
        ...
        mMaxCollapsedLines = typedArray.getInt(R.styleable.ExpandableLayout_maxCollapsedLines, MAX_COLLAPSED_LINES);
        ...
        textCollapse = typedArray.getString(R.styleable.ExpandableLayout_textCollapse);
        textExpand = typedArray.getString(R.styleable.ExpandableLayout_textExpand);

        if (mExpandDrawable == null) {
            mExpandDrawable = ContextCompat.getDrawable(getContext(),R.mipmap.img_drop_up);
        }
        if (mCollapseDrawable == null) {
            mCollapseDrawable = ContextCompat.getDrawable(getContext(), R.mipmap.img_drop_down);
        }
        ...
        contentTextColor = typedArray.getColor(R.styleable.ExpandableLayout_contentTextColor, ContextCompat.getColor(getContext(), android.R.color.darker_gray));
        contentTextSize = typedArray.getDimension(R.styleable.ExpandableLayout_contentTextSize, sp2px(getContext(),14));

        collapseExpandTextColor = typedArray.getColor(R.styleable.ExpandableLayout_collapseExpandTextColor, ContextCompat.getColor(getContext(), R.color.colorPrimary));
        collapseExpandTextSize = typedArray.getDimension(R.styleable.ExpandableLayout_collapseExpandTextSize, sp2px(getContext(),14));

        grarity = typedArray.getInt(R.styleable.ExpandableLayout_collapseExpandGrarity, Gravity.CENTER);
        drawableGrarity=typedArray.getInt(R.styleable.ExpandableLayout_drawableGrarity, Gravity.RIGHT);
...
//            mTvContent.setMaxLines(mMaxCollapsedLines);
    ...
    private void setDrawbleAndText(){
        ...
        }else if (Gravity.RIGHT==drawableGrarity){
            mTvExpandCollapse.setCompoundDrawablesWithIntrinsicBounds(null,null,mCollapsed ? mCollapseDrawable : mExpandDrawable,null);
        }else if (Gravity.TOP==drawableGrarity){
            mTvExpandCollapse.setCompoundDrawablesWithIntrinsicBounds(null,mCollapsed ? mCollapseDrawable : mExpandDrawable,null,null);
        }else if (Gravity.BOTTOM==drawableGrarity){
            mTvExpandCollapse.setCompoundDrawablesWithIntrinsicBounds(null,null,null,mCollapsed ? mCollapseDrawable : mExpandDrawable);
        }
        mTvExpandCollapse.setText(mCollapsed ? "展开" : "收起");
    }
    ...
    public void setText( CharSequence text,int position) {
        ...
        //设置收起/展开图标和文字
        setDrawbleAndText();
        mTvExpandCollapse.setText(mCollapsed ? "收起":"展开");
