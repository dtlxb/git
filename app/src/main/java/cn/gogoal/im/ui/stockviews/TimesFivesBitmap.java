...
public class TimesFivesBitmap {
    ...
    public HashMap<String, StockMinuteData> DetailMap = new HashMap<>();
    ...
    private void drawLines(Canvas canvas) {
        ...
        for (int i = 0; i < timesList.size(); i++) {
            ...
            DetailMap.put(StringUtils.save2Significand(uperBlueX), fenshiData);
            float endBlueY = (float) (uperBottom - (StringUtils.getDouble(DetailMap.get(StringUtils.save2Significand(uperBlueX)).getPrice())
                    + uperHalfHigh - initialWeightedIndex)
                    * uperRate);
            if (i == timesList.size() - 1) {
                endBlueLastX = uperBlueX;
            }
            float endYellowY = (float) (uperBottom - (StringUtils.getDouble(DetailMap.get(StringUtils.save2Significand(uperBlueX)).getAvg_price())
                    + uperHalfHigh - initialWeightedIndex) * uperRate);
            //五日图绘制
            if (!isTimes) {
                time_line_path.lineTo(uperBlueX, endBlueY);
                avg_line_path.lineTo(uperBlueX, endYellowY);
            ...
            if (isTimes) {
                if (cut == position) {
                    ...
                    avg_line_path.moveTo(uperBlueX, endYellowY);
