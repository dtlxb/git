 ...
 * Staff_id 1375
 * phone 18930640263
 * description :方便使用的RecyclerView（默认是竖起列表）不需要管理LayouManger，可以通过xml文件或setter方法设置
 */
public class EasyRecyclerView extends RecyclerView {
    ...
    public EasyRecyclerView(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < typedArray.getIndexCount(); i++) {
            ...
            if (attr == R.styleable.EasyRecyclerView_rv_type) {
                type = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.EasyRecyclerView_rv_orientation) {
                orientation = typedArray.getInt(attr, 0);
            } else if (attr == R.styleable.EasyRecyclerView_rv_column) {
                column = typedArray.getInt(attr, 1);
            } else if (attr == R.styleable.EasyRecyclerView_rv_divider_size) {
                dividerSize = (int) typedArray.getDimension(attr, 0f);
            } else if (attr == R.styleable.EasyRecyclerView_rv_divider_drawable) {
                dividerDrawable = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.EasyRecyclerView_rv_divider_color) {
                dividerColor = typedArray.getColor(attr, Color.BLACK);
            } else if (attr == R.styleable.EasyRecyclerView_rv_default_animator_open) {
    ...
    /**
     * 提醒EasyRecyclerView，当前的列表样式已经更改（一般是用代码动态修改了type和orientation后调用，如果修改了分割线样式，也需要调用该方法进行刷新）
     */
    ...
    class RecyclerViewListener extends RecyclerView.OnScrollListener {
        ...
        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
            ...
            if (move && newState == RecyclerView.SCROLL_STATE_IDLE) {
                ...
                if (0 <= n && n < EasyRecyclerView.this.getChildCount()) {
                    int top = EasyRecyclerView.this.getChildAt(n).getTop();
                    EasyRecyclerView.this.smoothScrollBy(0, top);
                }
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (move) {
                ...
                if (0 <= n && n < EasyRecyclerView.this.getChildCount()) {
                    int top = EasyRecyclerView.this.getChildAt(n).getTop();
                    EasyRecyclerView.this.scrollBy(0, top);
                }
            }
        }
    }

    /*================== EasyRecyclerView的滚动事件拓展 begin ==================*/
    ...
    /**
     * EasyRecyclerView的滚动事件拓展（原滚动事件被用于平滑滚动）
     */
    ...
    class LQRItemDecoration extends ItemDecoration {
        ...
        public LQRItemDecoration(Context context, int orientation, int dividerSize, int dividerColor, Drawable dividerDrawable) {
            ...
            if (dividerDrawable == null) {
                ...
                if (mDividerSize == 0) {
                    if (mOrientation == EasyRecyclerView.ORIENTATION_VERTICAL) {
