package cn.gogoal.im.adapter.recycleviewAdapterHelper.retry;
 ...
 */
public abstract class EasyAdapterForRecyclerView<T> extends RecyclerView.Adapter<EasyViewHolderForRecyclerView> {

    private Context mContext;
    private int mDefaultLayoutId = 0;
    private List<T> mData;
    private EasyHeaderAndFooterAdapter mHeaderAndFooterAdapter;

    /**
     * 当使用多种itemType时，最好使用这种构造方法
     */
    public EasyAdapterForRecyclerView(Context context, List<T> data) {
        mContext = context;
        mData = data;
    }
     ...
     */
    public EasyAdapterForRecyclerView(Context context, int defaultLayoutId, List<T> data) {
        this(context,data);
    ...
    public int getItemViewType(int position) {
        if (mDefaultLayoutId == 0) {
            throw new RuntimeException("请在 " + this.getClass().getSimpleName() + " 中重写 getItemViewType 方法返回布局资源 id，或者使用 " + this.getClass().getSimpleName() + " 三个参数的构造方法 EasyAdapterForRecyclerView(Context context, int defaultLayoutId, List<T> data)");
        }
        return mDefaultLayoutId;
    }

    @Override
    public EasyViewHolderForRecyclerView onCreateViewHolder(ViewGroup parent, int viewType) {
        return new EasyViewHolderForRecyclerView(mContext, View.inflate(mContext, viewType, null));
    }

    @Override
    public void onBindViewHolder(EasyViewHolderForRecyclerView holder, int position) {
        convert(holder, mData.get(position), position);
    }

//    @Override
//    public void onBindViewHolder(EasyViewHolderForRecyclerView holder, int position, List<Object> payloads) {
//        convert(holder, mData.get(position), position);
//    }
    ...
    public abstract void convert(EasyViewHolderForRecyclerView helper, T item, int position);
    ...
    public EasyHeaderAndFooterAdapter getHeaderAndFooterAdapter() {
        if (mHeaderAndFooterAdapter == null) {
            synchronized (EasyHeaderAndFooterAdapter.class) {
                if (mHeaderAndFooterAdapter == null) {
                    mHeaderAndFooterAdapter = new EasyHeaderAndFooterAdapter(this);
                }
            }
        }
