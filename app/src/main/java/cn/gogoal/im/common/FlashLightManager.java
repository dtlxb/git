 ...
 * EmployeeNumber 1375
 * Phone 18930640263
 * Description :闪光灯管理工具类
 *              如果配合拍照使用, 则无需调用init()方法, 直接使用turnLightOnCamera(Camera c) turnLightOffCamera(Camera c)
...
public class FlashLightManager {
    ...
    public void turnOn() {
        if (!isSupportFlash()) {
            UIHelper.toast(context,"设备不支持闪光灯！");
            return;
        }
        if (isOpenFlash) {
            return;
            ...
            private void _turnLightOnCamera2() {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    if (ActivityCompat.checkSelfPermission(context, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                        UIHelper.toast(context,"应用未开启访问相机权限！");
                        return;
                    }
                    try {
                        ...
                        UIHelper.toast(context,"开启失败：" + e.getMessage());
                    }
                }
            }
        }._turnLightOnCamera2();
            ...
            private void _initCamera2() {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    try {
                        ...
                        UIHelper.toast(context,"初始化失败：" + e.getMessage());
                    }
                }
            }
        }._initCamera2();
