...
public class VoiceManager {
                    ...
                    case MSG_TIME_INTERVAL:
                        if (mDeviceState == MEDIA_STATE_RECORD_DOING) {
                            ...
                            if (voiceRecordCallBack != null) {
                                voiceRecordCallBack.recDoing(mRecTimeSum, String.format(
                        ...
                        //播放
                            ...
                            //回调播放进度
                            if (voicePlayCallBack != null) {
                                voicePlayCallBack.playDoing(current / 1000, String.format(
                                        Locale.getDefault(),"%02d:%02d:%02d",
                                        ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));
                            }
                            mHandler.sendEmptyMessageDelayed(MSG_TIME_INTERVAL, 1000);
        ...
        if (mSBPlayProgress != null) {
            mSBPlayProgress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                ...
                public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                    ...
                    //播放进度
                    if (voicePlayCallBack != null) {
                        voicePlayCallBack.playDoing(progress / 1000, String.format(
                                Locale.getDefault(),"%02d:%02d:%02d",
                                ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond));
                    }
                }
     ...
     * 开始播放（外部调）
     */
    public void startPlay(Uri fileUri) {
        if (fileUri==null) {
            if (voicePlayCallBack != null) {
                voicePlayCallBack.playFinish();
            }
    ...
    private void startPlay(boolean init) {
        try {
            ...
            if (prepareMedia(mMediaPlayer, playFilePath)) {
                ...
                String voiceLength = String.format(Locale.getDefault(),"%02d:%02d:%02d",
                        ts.mSpanHour, ts.mSpanMinute, ts.mSpanSecond);
                //播放进度回调
                if (voicePlayCallBack != null) {
     ...
     * @return
     */
    private boolean prepareMedia(MediaPlayer mp, Uri fileUri) {
        boolean result = false;
        try {
            mp.setDataSource(mContext,fileUri);
            mp.prepare();
            result = true;
        } catch (Exception e) {
        }
