...
public class bookActivity extends AppCompatActivity {
    ...
    private void refresh(){
		new Thread(new Runnable(){
			...
			public void run() {
				try {
					URL url = new URL("http://"+ip+"/rest/getUsers");
					...
					for (int i = 0; i < mres.length(); i++) {
						...
						User booktmp=new User();
						booktmp.setUsername(thisuser.getString("username"));
						booktmp.setPassword(thisuser.getString("password"));

						booktmp.setId(Integer.parseInt(thisuser.getString("id")));
						users.add(booktmp);
					}
    ...
    private class MyAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent)
        {
            ...
            item.strTitle.setText(users.get(position).getUsername());
            item.strAuthor.setText(users.get(position).getPassword());

            item.strID.setText(users.get(position).getId()+"");
			//修改
